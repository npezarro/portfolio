...
const projects = [
  // ---- 01: Live Products ----
  {
    ...
    visual: 'screenshot',
    screenshotFile: 'quickclaude-landing.png',
    screenshotUrl: 'quick-claude — Landing Page',
    prev: null,
    next: 'claudenet',
  },
  {
    ...
    visual: 'screenshot',
    screenshotFile: 'claudenet-thread.png',
    screenshotUrl: 'claudenet — Thread View',
    prev: 'quick-claude',
    next: 'shopper',
  },
  {
    ...
    features: [
      ...
      'Skeptic-first research methodology with evidence-based recommendations',
      'Follow-up Q&A with conversational threading',
      'Shareable public links for completed guides',
  ...
  },
  {
    ...
    features: [
      ...
      'Card portfolio transfer partner cross-referencing',
      'Cents-per-point valuation calculations',
      'Hotel credit stacking (FHR, free night certs)',
      'Sweet spot detection for high-value redemptions',
      'Docker-bridged Claude CLI with travel-specific system prompt',
    ],
    visual: 'screenshot',
...
<span class="dim">[2026-05-12 08:12:45]</span> Usage: 5h 34% | 7d 28%`,
  ...
  },
  {
    ...
    github: 'https://github.com/npezarro/openAgentGuidance',
    desc: 'The instruction harness every agent I run shares. An always-loaded ruleset kept under a hard line budget, on-demand guidance files that each declare their own load trigger, git and session hooks that mechanically enforce the rules a model would otherwise forget, and subagent definitions. The linked repo is the screened public mirror: 19 guidance files, the hook set, and 8 subagent profiles.',
    features: [
      'Hierarchical instruction system (global → project → session)',
      'Always-loaded ruleset under a hard line budget',
      '19 on-demand guidance files, each with a Load when: trigger',
      'Git and session hooks that enforce rules mechanically',
      '8 subagent definitions (reviewer, debugger, architect, security, verifier)',
      'Automatic learning propagation from corrections',
      'Allowlist-gated publishing with a secret scanner and self-test',
    ],
    visual: 'arch',
    archData: {
      rows: [
        { boxes: [{ text: 'agent.md', cls: 'accent' }], label: 'Global Rules' },
        { boxes: [{ text: 'Guidance Files', cls: 'cool' }, { text: 'Hooks', cls: 'cool' }, { text: 'Subagents', cls: 'cool' }], label: 'Topic Layer' },
...
function generatePage(p) {
  ...
  } else if (p.visual === 'screenshot') {
    visualHtml = `
      <div class="project-visual">
        <div class="mockup-browser">
          <div class="mockup-browser-bar">
            <div class="mockup-browser-dots"><span></span><span></span><span></span></div>
            <div class="mockup-browser-url">${p.screenshotUrl || ''}</div>
          </div>
          <div class="mockup-browser-body" style="padding:0;overflow:hidden;">
            <img src="../assets/screenshots/${p.screenshotFile}" alt="${p.name} screenshot" style="width:100%;display:block;">
          </div>
        </div>
      </div>`;
